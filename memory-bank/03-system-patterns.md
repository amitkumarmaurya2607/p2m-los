...
- **Status timeline** — `ProfileStatusStep` matches `TrackApplicationV2` exactly: `h-5 w-5 rounded-full border-4` dot with a 4px halo (`shadow-[var(--shadow-green-ring)]` for done, `shadow-[var(--shadow-purple-ring)]` for active), `home-green` / `home-purple` / `border-medium` colors, `w-0.5` vertical line. The active step shows a bordered callout with a `Shield` icon. API is `state: "done" | "active" | "pending"` plus `isLast?: boolean`. The two halo shadow tokens (`--shadow-green-ring` / `--shadow-purple-ring`) are defined in `src/app/globals.css` and are the only way to render the brand-color halos in the project.
