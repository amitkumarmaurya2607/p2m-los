...
import { getLoanPrograms, submitApplication, getApplicationStatus } from "@/lib/services/apply.service";
...
export async function getApplicationStatusAction(id: string) {
  try {
    ...
    if (result.code !== "0000") return { error: result.message || "Failed to fetch application status" };
    return { success: true as const, data: result.data };
  } catch (err) {
    rethrowIfRedirect(err);
