    ...
    <div className="w-full">
      <div className="relative z-20">
        {label && (
          <label
            htmlFor={id}
            className={`absolute z-10 pointer-events-none font-semibold transition-all duration-200 ${isActive
              ? "left-5 top-2 text-[11px] text-primary"
              : `${leftIcon ? "left-12" : "left-5"} top-1/2 -translate-y-1/2 text-xs text-muted-foreground`
              }`}
          >
            {label}
        ...
        <DatePicker
          ...
          weekDayClassName={() => "custom-weekday"}
          monthClassName={() => "custom-month"}
          className={`h-16 w-full rounded-2xl bg-[#F8FAFC] px-5 pb-2 pt-6 text-sm text-foreground shadow-[0px_1px_3px_rgba(0,0,0,0.1),0px_1px_2px_-1px_rgba(0,0,0,0.1)] transition-all duration-200 focus:outline-none ${leftIcon ? "pl-12" : ""
            } ${rightIcon ? "pr-12" : ""} ${error
