  ...
  return (
    <>
      <footer className="bg-dark-navy px-5 py-10 font-sans md:px-10 lg:px-20">
        <div
          className="mx-auto h-px max-w-[1013px] bg-gradient-to-r from-transparent via-secondary/50
            to-transparent"
        />
        ...
        <div className="mx-auto mt-8 max-w-[var(--max-width-section)]">
          <div className="grid gap-10 lg:grid-cols-[1.4fr_0.6fr_0.7fr_1.4fr] lg:gap-12">
            ...
            <div>
              <div className="flex items-center gap-2">
                <span
                  className="flex h-8 w-8 items-center justify-center rounded-[10px] bg-secondary
              ...
              <p className="mt-6 max-w-[379px] text-[16px] leading-[26px] text-text-on-dark-muted">
                India's premium digital lending platform. Fast, transparent, and built to fuel your
                ambitions without the traditional banking stress.
              </p>

              <div className="mt-9 flex items-center gap-4">
                {[FacebookIcon, InnkedIcon, InstagramIcon, TwittarIcon].map((Icon, index) => (
                  <a
                    key={index}
                    href="#"
                    className="flex h-10 w-10 items-center justify-center rounded-full bg-white/5
                      text-text-on-dark-muted transition hover:bg-white/10 hover:text-white"
                  >
            ...
            {/* Stay Updated */}
            <div>
              <h4 className="text-[16px] font-bold leading-6 text-white">Stay Updated</h4>

              <p className="mt-6 max-w-[379px] text-[14px] leading-5 text-text-on-dark-muted">
                Get the latest financial insights and offers directly in your inbox.
              </p>

              <form
                className="mt-5 flex h-[50px] rounded-full border border-white/10 bg-white/5 p-1
                  overflow-hidden"
              >
                <input
                  type="email"
                  placeholder="Enter your email"
                  className="min-w-0 flex-1 bg-transparent px-4 text-[14px] text-white outline-none
                    placeholder:text-text-muted-dark"
                />
                <button
                  type="submit"
                  className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full
                    bg-secondary text-white hover:bg-secondary-light transition-colors"
                >
                  <ArrowRight size={16} />
                </button>
              </form>
              ...
              <div
                className="mt-6 flex items-center gap-2 rounded-[14px] border border-white/5
                  bg-white/5 p-3"
              >
          ...
          {/* Bottom */}
          <div
            className="mt-16 flex flex-col gap-5 border-t border-white/10 pt-8 text-[14px] leading-5
              text-text-muted-dark md:flex-row md:items-center md:justify-between"
          >
            <p>
      ...
      <ul className="mt-6 space-y-3">
        {links.map((link) => (
          <li key={link.label}>
            <Link
              href={link.href}
              className="text-[14px] font-medium leading-5 text-text-on-dark-muted hover:text-white
                transition-colors"
            >
              {link.label}
            </Link>
