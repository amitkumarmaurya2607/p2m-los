...
function SummaryCard({
  ...
  return (
    ...
    >
      ...
      <p
        className={`mt-1.5 break-words text-lg font-extrabold leading-6 sm:mt-2 sm:text-2xl
          sm:leading-8 ${green ? "text-[#00C89C]" : "text-[#3737C1]"}`}
      >
        {formatValue(value)}
      </p>
    </div>
...
function BankVerifiedBadge() {
  return (
    <div
      className="flex h-[38px] w-full items-center justify-center gap-2 rounded-[14px] border
        border-[#00C89C]/20 bg-[#00C89C]/10 px-4 py-2 sm:w-fit"
    >
  ...
  return (
    <aside
      className="w-full rounded-2xl border border-[#3737C1]/10 bg-[#3737C1]/5 p-4 sm:rounded-3xl
        sm:p-6 lg:min-h-[420px] lg:max-w-[306px] lg:p-8"
    >
      ...
      >
        ...
        <h4
          className="mt-4 break-words text-sm font-bold leading-5 text-[#0F172B] sm:text-base
            sm:leading-6"
        >
        ...
        <p
          className="mt-2 break-words text-sm font-medium leading-6 text-[#45556C]
            sm:leading-[23px]"
        >
      ...
      <div className="mt-5 space-y-3 sm:mt-6">
        {verificationItems.map((item) => (
          <div
            key={item.label}
            className="flex items-center justify-between rounded-xl border border-[#F1F5F9] bg-white
              px-3 py-3"
          >
        ...
        <div className="space-y-5 sm:space-y-8">
          ...
          >
            <div className="mt-5 grid grid-cols-1 gap-x-8 gap-y-5 sm:mt-6 sm:grid-cols-2 sm:gap-y-6">
              ...
              <InfoField
                ...
                value={formatValue(
                  bank?.accountHolderName || bank?.beneficiaryName || bank?.holderName,
                )}
              />
              ...
              <InfoField
                ...
                icon={<CreditCard className="h-3.5 w-3.5 text-[#90A1B9] sm:h-4 sm:w-4" />}
              />

              <InfoField label="IFSC Code" value={formatValue(ifscCode)} />

              <InfoField label="Account Type" value={formatValue(accountType)} />

              <InfoField label="Branch" value={formatValue(bank?.branchName || bank?.branch)} />
            </div>
          </WhiteInfoCard>
          ...
          >
            <div className="mt-5 grid grid-cols-1 gap-x-8 gap-y-5 sm:mt-6 sm:grid-cols-2 sm:gap-y-6">
              ...
              <InfoField label="Average Balance" value={formatAmount(bank?.averageBalance)} />

              <InfoField
                label="Statement Status"
                value={formatValue(bank?.statementStatus || bank?.bankStatementStatus)}
              />

              <InfoField
                label="Verification Status"
                value={formatValue(bank?.verificationStatus || bank?.status)}
                icon={<BadgeCheck className="h-3.5 w-3.5 text-[#00C89C] sm:h-4 sm:w-4" />}
              />
            </div>
          </WhiteInfoCard>
        </div>
