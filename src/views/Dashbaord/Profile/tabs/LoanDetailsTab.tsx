    ...
    return (
        <div className="rounded-2xl border border-[#F1F5F9] bg-[#F8FAFC] px-4 py-3">
            <div className="mb-1 flex items-center gap-2 text-[#3737C1]">
                ...
                <p className="text-[10px] font-bold uppercase leading-4 tracking-[0.45px] text-[#90A1B9] sm:text-[11px]">
                    {label}
                </p>
            </div>

            <p className="break-words text-sm font-bold leading-5 text-[#1D293D] sm:text-base sm:leading-6">
                {value}
            </p>
        </div>
    );
...
function LoanDetailsLoading() {
    return (
        <div className="space-y-3 rounded-2xl border border-[#F1F5F9] bg-white p-4">
            ...
            <p className="pt-1 text-center text-xs font-medium text-[#62748E]">
...
export default function LoanDetailsTab() {
    ...
    const [detailsByLoanId, setDetailsByLoanId] = useState<
        Record<string, DetailsEntry>
    >({});
    ...
    return (
        <div className="mx-auto w-full">
                ...
                <div className="mt-5 grid gap-5 sm:mt-8 sm:gap-6">
                    {loans.map((item) => {
                        ...
                        return (
                            <div
                                key={id}
                                className="overflow-hidden rounded-2xl border border-[#F1F5F9] bg-white shadow-[0px_8px_32px_-12px_rgba(0,0,0,0.05)] transition-all hover:shadow-[0px_16px_40px_-18px_rgba(0,0,0,0.18)] sm:rounded-3xl"
                            >
                                <div className="p-4 sm:p-6 lg:p-8">
                                    <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
                                        <div className="flex min-w-0 items-start gap-3 sm:gap-4">
                                            <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-[#3737C1]/10 sm:h-12 sm:w-12">
                                                <FileText className="h-5 w-5 text-[#3737C1] sm:h-6 sm:w-6" />
                                            </div>

                                            <div className="min-w-0">
                                                <div className="flex flex-wrap items-center gap-2">
                                                    <h3 className="break-all font-mono text-sm font-extrabold leading-5 text-[#0F172B] sm:text-base">
                                                        {loanId}
                                                    </h3>

                                                    {status && (
                                                        <span
                                                            className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[11px] font-bold leading-4 sm:px-3 sm:text-xs ${statusClass[status] ||
                                                ...
                                                {typeof item.purpose === "string" && item.purpose && (
                                                    <p className="mt-1 text-xs font-medium leading-5 text-[#62748E] sm:text-sm">
                                                        {item.purpose}
                                                    </p>
                                                )}
                                            </div>
                                        ...
                                        <div className="grid gap-3 sm:grid-cols-3 lg:w-[460px]">
                                            ...
                                            <SummaryMiniCard
                                                label="Applied On"
                                                value={formatDate(
                                                    item.applicationDate as string | null | undefined
                                                )}
                                            ...
                                            <SummaryMiniCard
                                                label="Due Date"
                                                value={formatDate(
                                                    (item.loanDetails as any)?.dueDate as
                                                    | string
                                                    | null
                                                    | undefined
                                                )}
                                    ...
                                    <div className="mt-5 flex flex-col gap-3 border-t border-[#F1F5F9] pt-4 sm:flex-row sm:items-center sm:justify-between">
                                        ...
                                        <button
                                            ...
                                            className="inline-flex h-10 w-full items-center justify-center gap-2 rounded-xl border border-[#3737C1]/20 bg-[#3737C1]/10 px-4 text-sm font-bold text-[#3737C1] transition hover:bg-[#3737C1] hover:text-white sm:w-fit"
                                        >
                                    ...
                                    <div className="overflow-hidden">
                                        <div className="border-t border-[#F1F5F9] bg-[#F8FAFC] p-4 sm:p-6 lg:p-8">
                                            ...
                                            >
                                                ...
                                                <InfoField
                                                    icon={
                                                        <CalendarDays className="h-4 w-4 text-[#3737C1]" />
                                                    }
                                                    label="Created"
                                                ...
                                                <InfoField
                                                    ...
                                                    value={formatAmount(
                                                        (item as any)?.disbursement?.netAmount
                                                    )}
                                                />

                                                <InfoField
                                                    icon={
                                                        <CircleDollarSign className="h-4 w-4 text-[#3737C1]" />
                                                    }
                                                    label="Total Obligation"
                                                    value={formatAmount(
                                                        (item as any)?.repayment?.totalObligation
                                                    )}
                                                />
                                            </DetailSection>
            ...
            <div className="mt-5 flex items-start gap-2 rounded-2xl border border-[#3737C1]/20 bg-[#3737C1]/10 px-4 py-4 text-sm font-medium leading-6 text-[#1D293D] sm:mt-6">
                <Info className="mt-0.5 h-4 w-4 shrink-0 text-[#3737C1]" />

                <p>
                    <span className="font-bold">Note:</span> Loan list is fetched on load.
                    Full details are fetched once per loan and cached.
                </p>
            </div>
        </div>
    );
        ...
        <div className="space-y-5 sm:space-y-6">
            ...
            >
                ...
                <InfoField
                    ...
                    label="Processing"
                    value={formatAmount(
                        processingDeduction?.total ?? data.disbursement?.processing_fee
                    )}
                />
...
function DetailSection({
    ...
    return (
        <section className="rounded-2xl border border-[#E2E8F0] bg-white p-4 shadow-[0px_8px_32px_-12px_rgba(0,0,0,0.05)] sm:rounded-3xl sm:p-5 lg:p-6">
            <div className="mb-5 flex flex-col gap-3 border-b border-[#F1F5F9] pb-4 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex min-w-0 items-start gap-3">
                    {icon && (
                        <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-[#3737C1]/10">
                            {icon}
                        </div>
                    )}

                    <div className="min-w-0">
                        <h3 className="text-sm font-extrabold leading-5 text-[#0F172B] sm:text-base sm:leading-6">
                            {title}
                        </h3>

                        {description && (
                            <p className="mt-0.5 text-xs font-medium leading-5 text-[#62748E]">
                                {description}
                            </p>
                        )}
                    </div>
