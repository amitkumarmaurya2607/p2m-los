...
import { AlertCircle, CheckCircle2, Clock3, IndianRupee, Loader2, XCircle } from "lucide-react";
...
function LoanApplicationCard({
  ...
  const applicationId = loanData?.applicationId || loanData?.formattedLoanId || loanData?.id || "-";

  const loanAmount = loanData?.loanAmount || loanData?.amount || 0;

  const appliedDate = loanData?.applicationDate || loanData?.createdAt || loanData?.appliedDate;

  const expectedDisbursal =
    loanData?.disbursementDate || loanData?.dueDate || loanData?.loanDetails?.expectedDisbursalDate;

  const agreementStatus =
    typeof loanData?.agreement === "string" ? loanData?.agreement : loanData?.agreement?.status;
    ...
    <div className="w-full min-w-0">
      ...
      >
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between sm:gap-4">
          <div
            className="w-fit max-w-full rounded-[10px] border border-[rgba(55,55,193,0.2)]
              bg-[rgba(55,55,193,0.1)] px-3 py-1.5 sm:px-4 sm:py-2"
          >
            <span
              className="break-all text-[11px] font-bold uppercase leading-4 tracking-[1px]
                text-[#3737C1] sm:text-sm sm:leading-5 sm:tracking-[1.4px]"
            >
        ...
        <div
          className="mt-6 grid grid-cols-2 gap-x-4 gap-y-6 sm:mt-8 sm:grid-cols-2 sm:gap-x-6
            sm:gap-y-8 lg:grid-cols-3"
        >
          <InfoBlock
            label="Loan Amount"
...
function StatusNotice({ status, errorMsg }: { status: string; errorMsg?: string }) {
  const notice = {
    loading: {
  ...
  return (
    <div
      className={`mt-4 flex items-start gap-2.5 rounded-2xl border p-3 sm:mt-6 sm:gap-3 sm:p-4
        ${notice.className}`}
    >
      <AlertCircle size={18} className={`mt-0.5 shrink-0 sm:size-5 ${notice.iconClass}`} />
...
function ProgressTracker({ status }: { status: string }) {
  ...
  return (
    <div
      className="w-full rounded-2xl border border-[#F1F5F9] bg-white p-4
        shadow-[0px_8px_32px_-12px_rgba(0,0,0,0.05)] sm:rounded-3xl sm:p-6 lg:max-w-[306px] lg:p-8"
    >
      <h3
        className="border-b border-[#F1F5F9] pb-3 text-base font-bold leading-6 text-[#0F172B]
          sm:pb-4 sm:text-lg sm:leading-7"
      >
        Progress Tracker
      </h3>
        ...
        {steps.map((step) => {
          ...
          return (
            <div key={step.title} className="relative pb-6 last:pb-0 sm:pb-8">
              <span
                className={`absolute -left-[29px] top-0 h-3.5 w-3.5 rounded-full border-[3px]
                sm:-left-[33px] sm:h-4 sm:w-4 sm:border-4 ${dotClass}`}
              />
              ...
              <p
                className={`pt-0.5 text-[11px] font-medium leading-4 sm:pt-1 sm:text-xs
                ${statusClass}`}
              >
                {step.statusText}
              </p>
...
export default function LoanApplication() {
  const { status, loanData, errorMsg, currentStatus, getDetails, loading } = useProfile();

  const noActiveLoan = status !== "loading" && !loading && !loanData?.status;

  if (status === "loading" || loading) return <LoanApplicationSkeleton />;

  return (
    <div className="mx-auto w-full">
      <PageHeader loading={loading} onButtonClick={getDetails} />
