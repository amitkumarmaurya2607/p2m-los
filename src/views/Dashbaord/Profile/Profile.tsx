...
import { User, FileCheck2, CreditCard, CheckCircle2, Briefcase, Landmark } from "lucide-react";
...
const Profile = () => {
  ...
  const fullName = useMemo(() => {
    const name = [profileData?.firstName, profileData?.middleName, profileData?.lastName]
      .filter(Boolean)
      .join(" ");
  ...
  useEffect(() => {
    const validKeys = new Set<TabKey>(["LoanApplication", ...filteredTabs.map((t) => t.key)]);
          ...
          >
            ...
            <div className="mb-3 rounded-2xl border border-border bg-background p-4">
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  <h3 className="truncate text-sm font-bold text-text-heading">{fullName}</h3>
              ...
              <div className="mt-4 rounded-xl bg-primary-muted px-3 py-2">
                ...
                <p className="mt-0.5 text-sm font-bold text-primary">{"In Progress"}</p>
              </div>
            </div>
