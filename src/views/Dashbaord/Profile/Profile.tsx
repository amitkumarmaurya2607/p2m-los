...
const Field = ({ label, value, full = false }: { label: string; value: string; full?: boolean }) => (
  <div className={full ? "md:col-span-2" : ""}>
    <label className="mb-3 block text-base font-normal text-text-muted">
      {label}
    </label>
    <div className="h-10 rounded-xl border border-border bg-surface px-4 text-base leading-10 text-text-heading">
      {value}
    </div>
  </div>
);
...
const Profile = () => {
  ...
  const renderContent = () => {
    switch (activeTab) {
      case "Profile":
        return (
          <>
            <h2 className="mb-7 text-2xl font-normal text-text-heading">
              My Profile
            </h2>
      ...
      case "Delete Account":
        return (
          <div className="flex flex-col items-center justify-center py-16 text-center">
            <h3 className="mb-3 text-2xl font-normal text-destructive">
              Delete Account
            </h3>
  ...
  return (
    <div className="mx-auto min-h-screen w-full max-w-[var(--max-width-section)] px-5 text-text-heading md:px-8 lg:px-0">
      <div className="mb-7">
        <h1 className="text-3xl font-normal text-text-heading">
          Profile Settings
        </h1>
      ...
      <div className="flex flex-col gap-5 lg:flex-row">
        <aside className="w-full rounded-2xl border border-border-light bg-surface p-5 shadow-[var(--shadow-card)] lg:h-[292px] lg:w-[202px]">
            ...
            {menuItems.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`whitespace-nowrap rounded-lg px-3 py-[9px] text-left text-base leading-[22px] transition-colors lg:w-full ${isActive
        ...
        <main className="flex-1 rounded-2xl border border-border-light bg-surface px-4 py-6 shadow-[var(--shadow-card)] sm:px-6 lg:px-10">
          <section className="mb-[18px] rounded-xl border border-border bg-surface px-4 py-4 sm:px-5">
            <div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
              <div className="flex items-center gap-4">
                <div className="flex h-[100px] w-[100px] items-center justify-center rounded-full bg-primary-muted">
