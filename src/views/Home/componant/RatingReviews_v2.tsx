    ...
    <section className="relative overflow-hidden bg-white py-[60px]">
      <div className="mx-auto flex max-w-[1280px] flex-col items-center px-6 text-center">
        <div className="flex items-center gap-2">
          <h2 className="text-[36px] font-black leading-[40px] text-[#0F172A]">4.9</h2>

          <div className="flex flex-col items-start">
            <div className="flex gap-1">
              {Array.from({ length: 5 }).map((_, index) => (
                <Star key={index} className="h-5 w-5 fill-[#FF9F1C] text-[#FF9F1C]" />
              ))}
            </div>

            <p className="text-sm font-semibold text-[#62748E]">Google Rating</p>
          </div>
        </div>
      ...
      <div
        ref={scrollRef}
        className="mt-8 flex gap-6 overflow-x-auto scroll-smooth px-6 pb-4 [scrollbar-width:none]
          [&::-webkit-scrollbar]:hidden"
      >
        {duplicatedReviews.map((review, index) => (
          <div
            key={`${review.name}-${index}`}
            className="relative min-h-[225px] min-w-[350px] rounded-2xl border border-[#F1F5F9]
              bg-white px-[25px] pt-[24px] shadow-[0px_8px_30px_rgba(0,0,0,0.04)]"
            style={{
              borderTopWidth: "4px",
          ...
          >
            <div className="flex gap-1">
              {Array.from({ length: 5 }).map((_, i) => (
                <Star key={i} className="h-4 w-4 fill-[#FF9F1C] text-[#FF9F1C]" />
              ))}
            </div>
            ...
            <div className="mt-6 flex items-center gap-4">
              ...
              <div>
                <h4 className="text-[14px] font-bold leading-5 text-[#0F172A]">{review.name}</h4>

                <p className="text-[12px] font-medium text-[#62748E]">{review.role}</p>
              </div>
            </div>
          </div>
        ))}
      ...
      <div
        className="absolute bottom-[40px] left-[40px] h-3 w-3 rounded-full bg-[#00C89C]/40
          blur-[1px]"
      />
